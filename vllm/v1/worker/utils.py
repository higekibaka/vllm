...
def add_kv_sharing_layers_to_kv_cache_groups(
    ...
    for layer_name, target_layer_name in shared_kv_cache_layers.items():
        ...
        if isinstance(tgt_kv_cache_group.kv_cache_spec, UniformTypeKVCacheSpecs):
            # Shared layers do not allocate a separate KV cache, but attention
            # backend initialization still indexes per-layer specs by layer
            # name. Mirror the target layer's spec for grouped heterogeneous
            # models.
