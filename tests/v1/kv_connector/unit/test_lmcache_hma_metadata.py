...
try:
    ...
    pytest.skip(
        f"optional native LMCache adapter is unavailable: {exc}",
        allow_module_level=True,
    )

LMCacheConnectorV1Impl = vllm_v1_adapter.LMCacheConnectorV1Impl
ReqMeta = vllm_v1_adapter.ReqMeta
RequestTracker = vllm_v1_adapter.RequestTracker
...
def test_request_tracker_update_accepts_grouped_block_ids():
    ...
    tracker.update(new_token_ids=[12], new_block_ids=([2], [6]))

    assert tracker.token_ids == [10, 11, 12]
    assert tracker.allocated_block_ids_by_group == ([1, 2], [5, 6])


def test_request_tracker_update_accepts_flat_tuple_block_ids():
    tracker = RequestTracker(
        req_id="req",
        prompt_len=2,
        token_ids=[10, 11],
    ...
    tracker.update(new_token_ids=[12], new_block_ids=(2, 3))
...
def test_req_meta_uses_group_block_sizes_for_slot_mappings():
    ...
    assert all(
        mapping.shape[0] == 6805 for mapping in req.slot_mappings_by_layer.values()
    )
